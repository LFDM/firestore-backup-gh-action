...
sh -c "gcloud auth activate-service-account --key-file=$SERVICE_ACCOUNT_FILE --quiet"
rm "$SERVICE_ACCOUNT_FILE"

[ -z "$INPUT_COLLECTIONIDS" ] || COLLECTION_IDS="--collection-ids $INPUT_COLLECTIONIDS"

sh -c "gcloud beta firestore --project=$INPUT_PROJECTID export $INPUT_STORAGEBUCKET $COLLECTION_IDS --quiet"